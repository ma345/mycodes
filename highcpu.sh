...
---------------------------------
    ...
    function urls_by_ip() {
        local IP=$1
        tail -$HOW_MANY_ROWS $ACCESS_LOG | awk -v ip=$IP ' $1 ~ ip {freq[$7]++} END {for (x in freq) {print freq[x], x}}' | sort -rn | head -10
    }
    function ip_addresses_by_user_agent(){
        local USERAGENT_STRING="$1"
        local TOP_5_IPS="`tail  -$HOW_MANY_ROWS $ACCESS_LOG | grep "${USERAGENT_STRING}"  | awk '{freq[$1]++} END {for (x in freq) {print freq[x], x}}' | sort -rn | head -10`"
        echo "$TOP_5_IPS"
    }
    ####### RUN REPORTS #############
    ...
    appname
    title "top 10 URLs"
    TOP_5_URLS="`tail -$HOW_MANY_ROWS $ACCESS_LOG | awk '{freq[$7]++} END {for (x in freq) {print freq[x], x}}' | sort -rn | head -10`"
    echo "$TOP_5_URLS"
    title "top 10 URLS excluding POST data"
    TOP_5_URLS_WITHOUT_POST="`tail  -$HOW_MANY_ROWS $ACCESS_LOG | awk -F"[ ?]" '{freq[$7]++} END {for (x in freq) {print freq[x], x}}' | sort -rn | head -10`"
    echo "$TOP_5_URLS_WITHOUT_POST"
    title "top 10 IPs"
    TOP_5_IPS="`tail  -$HOW_MANY_ROWS $ACCESS_LOG | awk '{freq[$1]++} END {for (x in freq) {print freq[x], x}}' | sort -rn | head -10`"
    echo "$TOP_5_IPS"
    ...
    title "top 10 slow functions {Count of function, Function, Plugin}"
